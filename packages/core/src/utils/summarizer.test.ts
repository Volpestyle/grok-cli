...
describe('summarizers', () => {
  ...
  beforeEach(() => {
    ...
    mockGrokClient = new GrokClient(mockConfigInstance);
    (mockGrokClient.generateContent as Mock) = vi.fn();
  ...
  describe('summarizeToolOutput', () => {
    it('should return original text if it is shorter than maxLength', async () => {
      ...
      expect(mockGrokClient.generateContent).not.toHaveBeenCalled();
    });
    ...
    it('should call generateContent if text is longer than maxLength', async () => {
      ...
      (mockGrokClient.generateContent as Mock).mockResolvedValue({
      ...
      expect(mockGrokClient.generateContent).toHaveBeenCalledTimes(1);
    ...
    it('should return original text if generateContent throws an error', async () => {
      ...
      (mockGrokClient.generateContent as Mock).mockRejectedValue(error);
    ...
    it('should construct the correct prompt for summarization', async () => {
      ...
      await summarizeToolOutput(longText, mockGrokClient, abortSignal, 1000);
...
`;
      const calledWith = (mockGrokClient.generateContent as Mock).mock
        .calls[0];
