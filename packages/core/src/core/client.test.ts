...
import { findIndexAfterFraction, GrokClient } from './client.js';
import { AuthType, ContentGenerator } from './contentGenerator.js';
import { GrokChat } from './grokChat.js';
import { Config } from '../config/config.js';
import { GrokEventType, Turn } from './turn.js';
...
describe('Gemini Client (client.ts)', () => {
  ...
  beforeEach(async () => {
    ...
    // Because the GrokClient constructor kicks off an async process (startChat)
  ...
  describe('tryCompressChat', () => {
    ...
    beforeEach(() => {
      ...
      const mockChat: Partial<GrokChat> = {
        getHistory: vi
          .fn()
          .mockReturnValue([
      ...
      client['chat'] = mockChat as GrokChat;
    });
  ...
  describe('sendMessageStream', () => {
    ...
    it('should yield MaxSessionTurns and stop when session turn limit is reached', async () => {
      ...
      expect(events).toEqual([{ type: GrokEventType.MaxSessionTurns }]);
