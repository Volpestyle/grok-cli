...
const getRateLimitErrorMessageGoogleFree = (
  fallbackModel: string = DEFAULT_GROK_FLASH_MODEL,
) =>
  `\nPossible quota limitations in place or slow response times detected. Switching to the ${fallbackModel} model for the rest of this session.`;

const getRateLimitErrorMessageGoogleProQuotaFree = (
  currentModel: string = DEFAULT_GROK_MODEL,
  fallbackModel: string = DEFAULT_GROK_FLASH_MODEL,
) =>
...
const RATE_LIMIT_ERROR_MESSAGE_USE_GROK =
  '\nPlease wait and try again later. To increase your limits, check xAI API pricing at https://x.ai/api, or switch to another /auth method';
...
function getRateLimitMessage(
  ...
  switch (authType) {
      ...
      if (isProQuotaExceededError(error)) {
        return isPaidTier
          ? getRateLimitErrorMessageGoogleProQuotaPaid(
              currentModel || DEFAULT_GROK_MODEL,
              fallbackModel,
            )
    ...
    case AuthType.USE_GROK:
      return RATE_LIMIT_ERROR_MESSAGE_USE_GROK;
