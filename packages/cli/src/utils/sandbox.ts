...
import { SandboxConfig } from 'grok-cli-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'grok-cli-sandbox';
const SANDBOX_NETWORK_NAME = 'grok-cli-sandbox';
const SANDBOX_PROXY_NAME = 'grok-cli-sandbox-proxy';
...
export async function start_sandbox(
  ...
  // determine full path for grok-cli to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under grok-cli repo
  //
  // note this can only be done with binary linked from grok-cli repo
  if (process.env.BUILD_SANDBOX) {
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ...
        : 'Please check the image name, your network connection, or notify grok-cli-dev@xai.com if the issue persists.';
    console.error(
  ...
  // Remove legacy GEMINI_API_KEY support
  if (process.env.GOOGLE_API_KEY) {
    args.push('--env', `GOOGLE_API_KEY=${process.env.GOOGLE_API_KEY}`);
  }
  // copy GROK_API_KEY
  if (process.env.GROK_API_KEY) {
    args.push('--env', `GROK_API_KEY=${process.env.GROK_API_KEY}`);
  }
  ...
  // Remove legacy GEMINI_MODEL support
